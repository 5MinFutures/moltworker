...
app.use('*', async (c, next) => {
  ...
  // /telegram-webhook is called by Telegram’s servers and cannot complete CF Access auth,
  // so it must be exempted from the JWT requirement. Security for that route will be handled
  // by the Telegram secret token and bot token.
  const isPublicWebhook = url.pathname === '/telegram-webhook' || url.pathname.startsWith('/telegram-webhook/');
  if (c.env.DEV_MODE === 'true' || isPublicWebhook) {
    return next();
  }
